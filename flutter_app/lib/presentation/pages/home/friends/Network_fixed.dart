...
class _NetworkPageState extends ConsumerState<NetworkPage>
  ...
  Future<void> _fetchAvailableUsers() async {
    ...
    final url = Uri.parse('http://localhost:8000/api/users/available');
  ...
  Future<void> _fetchFriendsList() async {
    ...
    final url = Uri.parse('http://localhost:8000/api/users/friends');
    ...
    if (response.statusCode == 200) {
      ...
      print('Failed to fetch friends list: ${response.body}');
    }
  }
  ...
  Future<void> _sendFriendInvitation(int friendId) async {
    final authState = ref.read(authProvider);
    final token = authState.accessToken;
    ...
    final url = Uri.parse('http://localhost:8000/api/invitations/send');
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'receiver_id': friendId}),
      );
      ...
      if (response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                responseData['message'] ?? 'Invitation envoyée avec succès'),
            backgroundColor: const Color(0xFF2EE59D),
          ),
        );

        // Refresh the available users list to remove the invited user
        await _fetchAvailableUsers();
      } else if (response.statusCode == 400) {
        final errorData = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(errorData['message'] ?? 'Invitation déjà envoyée'),
